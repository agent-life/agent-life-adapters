...
| openclaw | ROOT_FILES (SOUL/IDENTITY/AGENTS/USER/TOOLS/HEARTBEAT/BOOTSTRAP/MEMORY .md), `memory/**`, every workspace `*.md` (scatter-capture rule, excl. `.git/`), include-list + sentinels, `~/.openclaw/openclaw.json` (agent set changes). Practical shape: one recursive workspace watch + the export-time exclusion filter, not per-glob watches (`export.rs:210-399`) |
