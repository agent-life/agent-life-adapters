//! Non-mutating plan for ZeroClaw's cross-runtime structured fallback.
...
/// workspace, credential target, or SQLite database.
pub(super) fn build<R: std::io::Read + std::io::Seek>(
    ...
    // The sidecar is not a file output, but parsing it now prevents an invalid
    // ZeroClaw archive from changing the workspace before brain restore fails.
    let _schema = super::read_schema_sidecar(alf)?;

    if let Some(identity) = identity {
        if let Some(prose) = identity.prose.as_ref() {
            if let Some(soul) = &prose.soul {
                plan.add(workspace, "SOUL.md", soul.as_bytes().to_vec())?;
            }
            if let Some(profile) = &prose.identity_profile {
                plan.add(workspace, "IDENTITY.md", profile.as_bytes().to_vec())?;
            }
            if let Some(instructions) = &prose.operating_instructions {
                plan.add(workspace, "AGENTS.md", instructions.as_bytes().to_vec())?;
            }
        } else if let Some(structured) = identity.structured.as_ref() {
            let name = structured
                .names
                .as_ref()
        ...
        if identity.source_format.as_deref() == Some("aieos") {
            if let Some(raw) = &identity.raw_source {
                plan.add(
                    workspace,
                    "identity.json",
    ...
    for record in &all_records {
        ...
        match record.namespace.as_str() {
            "core" => core_sections.push(record.content.clone()),
            "daily" => {
            ...
            "conversation" => {
                ...
                validate_memory_target(workspace, &key, "")?;
                daily_groups
                    .entry(key)
                    .or_default()
                    .push(record.content.clone());
            ...
            "session" => {
                let key = if !origin_file.is_empty() {
                    ...
                    format!("memory/session_{}.md", &record.id.to_string()[..8])
                };
                validate_memory_target(workspace, &key, origin_file)?;
                other_files
                    .entry(key)
