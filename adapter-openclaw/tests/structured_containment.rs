...
fn build_cross_runtime_archive(out_path: &Path, origin_file: &str) {
    ...
    .unwrap();
    fs::write(source.join("USER.md"), "# User\n").unwrap();
    ...
    OpenClawAdapter.export(&source, &source_archive).unwrap();
...
fn cross_import_rejects_unsafe_origin_file_without_mutation() {
    ...
    for origin_file in [
        ...
        let error = OpenClawAdapter.import(&archive, &workspace).unwrap_err();
        assert!(
...
fn cross_import_accepts_safe_origin_file() {
    ...
    OpenClawAdapter.import(&archive, &workspace).unwrap();
    assert_eq!(
