...
- **The watch loop — token-free, event-driven auto-sync.** While an MCP session keeps the server alive, a background loop marks sources dirty on filesystem events (plus a periodic rescan), debounces to the interval, captures on quiescence (a plain byte copy once the file has been still for the quiesce window — **SQLite stores get no special handling in v1**: a `.db` and its `-wal`/`-shm` sidecars are captured together as one near-consistent trio, and backup-API / `VACUUM INTO` row extraction is reserved for v2 because it would break raw fidelity), and syncs through the same path a tool call would — at zero token cost. Delta floor **1 min**, tracked-file snapshot-rollover floor **15 min** (default 1 h), ceiling **24 h**; catch-up dirty-scan on start; crash-safe by SIGKILL (atomic state + server-side sequence CAS); auto-recover-once-then-park for E4 (base deleted) / E7 (409 conflict) / E9 (poisoned base); per-agent advisory lock. Steer it with `alf_watch_set` or the map's `watch` block. **No daemon mode** — host-independent cadence stays with the CLI + cron / runtime hooks.
  > **Known limitation — a tracked-file change defers the whole tick.** While any dirty *tracked* source is still inside its interval, the loop defers **every** source's auto-sync, not just the tracked one. Because the catch-up scan on start marks all sources dirty, the tracked clock effectively begins at server start — so an `alf add` / `alf_track` (or any `.alf-include.json` / `.alf-sync-log.md` write) within the first tracked interval pauses that agent's auto-sync until the interval elapses (**up to 1 h at the default**, 15 min at the floor). Nothing is lost — the deferred sync captures every change when it fires — and a manual `alf sync` is unaffected. Lower `tracked_files_interval` via `alf_watch_set` if the pause matters to you. Letting untracked sources ride the delta cadence while a tracked source waits is a **v1.2** change to the shared engine gate, deliberately not made during the 1.1 train.
- **Supported runtimes get the MCP option on user machines** (design §7.W2). **Every adapter splits its surface into the same cadence channels:** broad memory/content sources on the normal delta cadence; `tracked-files` (include-list entries) and `tracked-controls` (`.alf-include.json`, `.alf-sync-log.md`) on `tracked_files_interval`, because a change to any of them forces a full-snapshot rollover; and an untracked `export-controls` spec for `.alfignore` (plus `.alf-map.json` on generic), which changes export selection but is not part of that rollover comparison. Per-adapter specifics: openclaw (one recursive workspace source for the scatter-capture Markdown surface, excluding the tracked roots so an ordinary Markdown edit keeps the normal cadence, + `~/.openclaw/openclaw.json`), zeroclaw (the `brain.db` `+wal/+shm` sidecar trio + `memory/` + ROOT_FILES + AIEOS `identity.json`), hermes (the per-profile allowlist + `state.db` trio + `skills/` + a `profiles/`-dir re-discovery hook that surfaces a new agent in `alf_agents_list` mid-session). No CLI-path behavior changes — `watch_paths` is only ever called by the MCP loop.
...
- **`.alf-include.json` is forward compatible.** Both the document and each entry now round-trip unknown fields, so an older `alf` that rewrites the list (every restore marks restored externals inert; every sync prunes missing files) no longer silently deletes fields a newer version wrote. This matters because the file travels inside the archive and mixed versions are routine — a runtime image bakes its `alf` at build time and lags the machine that wrote the list.
